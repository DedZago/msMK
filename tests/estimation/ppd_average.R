...
for(w in 1:length(dirs)){
    imgname = paste0(dirs[w], "/img/", "average_ppdSmooth-indTRUE-red-n250.png")
    if(!file.exists(imgname)){
        ...
        for(j in 1:nsim){
            cat("j:", j, "\n")
            load(paste0(dirs[w], "/i", j,"-indTRUE-n250.Rdata"))
            pred_list[[j]] = predictive_smooth_average(mcmc.msmk, y.stand, ylim = c(-4.5,4.5))
        }
        save(pred_list, file = paste0(dirs[w], "pred_list-indTRUE-n250.Rdata"))
        ...
        x = pred_list[[1]]$x1
        y = pred_list[[1]]$x2
        contour(x, y, pred, col = "darkorange", nlevels=35)
        densContour(df, xlim = c(min(x), max(x)), ylim = c(min(y), max(y)), nlevels=200, col = "black",
        add = TRUE)
        dev.off()
    }
}

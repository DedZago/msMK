...
)
{
    ...
    umat cluster = clusterAllocation(TH, SIG, prob, pi_s, y, indep);
    ...
    for(int i = 0; i < nsim; ++i){
        ...
        if(i == burnin){
            ...
            // Reset cpo because of burnin
            cpo_inv.fill(0.0);
        }
        pi_s = scaleProb(prob);
        // Rcout << "pi_s:\n";
        // Rcout << pi_s << endl;
        cluster = clusterAllocation(thetaNew, sigmaNew, prob, pi_s, y, cluster, indep, cpo_inv);
        // Rcout << "cluster:\n";
        // Rcout << cluster << endl;
    ...
    NumericVector cpo = (nsim - burnin) / cpo_inv;

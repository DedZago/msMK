...
for(n in 1:length(ns)){
    for(i in 1:length(dirs)){
        ...
        if(ns[n] != 1000){
            ...
            load(paste0(dirs[i], "/lpml-DP", "-n", ns[n],".Rdata"))
            temp = cbind(temp, lpml_mat[ , 1 ])
            temp = temp[ , c(1, 3, 4, 2) ]
            ...
            lpml_mean = apply(temp2, 2, mean) %>% round(3)
            lpml_sd = apply(temp2, 2, sd) %>% round(3)

            lpml_list[[(n-1)*length(dirs) + i]] = paste0(format(lpml_mean, nsmall = 1), " (", format(lpml_sd, nsmall=1), ")")
        } else{
            cat("Here\n")
            load(paste0(dirs[i], "/lpml-indTRUE.Rdata"))
            temp = lpml_mat
            load(paste0(dirs[i], "/lpml-indFALSE.Rdata"))
            temp = cbind(temp, lpml_mat[ , 1 ])
            load(paste0(dirs[i], "/lpml-DP-n1000.Rdata"))
            temp = cbind(temp, lpml_mat[ , 1 ])
            temp = temp[ , c(1, 3, 4, 2) ]
            colnames(temp) = c("Product", "Full covariance", "DP", "True")
            temp2 = temp[ , c(1,2,3) ] / temp[ , 4 ]
...
do.call(rbind, lpml_list) %>% 
    ...
    set_colnames(c("Product", "Full covariance", "DP")) %>% 
...
do.call(rbind, lpml_list) %>% 
    ...
    set_colnames(c("Product / True", "Full covariance / True", "DPM / True")) %>% 
